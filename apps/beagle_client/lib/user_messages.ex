defmodule UserMessages do
  ...
  import BeagleClient.Gettext
  	...
  	"""
	def const_server_down, do: gettext("Oops! It looks like our servers are down. We are working on bringing them back up as soon as possible.")
	...
	def resourceNotFound(params, type) do
		...
		gettext("Oh no! We could not find any ") <> type <> gettext(" associated with: ") <> key_error_message
	end
	...
	"""
	def const_register_success_response, do: gettext("Nice! We received your registration request. You should receive an email once its approved by one of our admins.")
	...
	"""
	def const_run_submmitted_message, do: gettext("Awesome! Your run has been successfully submitted")

	@doc """
	No metadata changes ready to publish
	"""
	def const_no_metadata_changes_ready, do: gettext("Hmmm, it looks like there are no changes ready to publish")

	@doc """
	No QC report data to show
	"""
	def const_no_qc_report_data, do: gettext("Hmmm, there is no qc report data to show for this sample")
